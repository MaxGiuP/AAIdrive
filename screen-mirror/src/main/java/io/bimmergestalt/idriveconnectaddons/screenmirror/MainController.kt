...
class MainController(val activity: Activity) {
    ...
    fun startAndroidAuto() {
        val receiver = Intent(Intent.ACTION_VIEW, Uri.parse("headunit://selfmode"))
            .setPackage("com.andrerinas.headunitrevived")
        if (activity.packageManager.resolveActivity(receiver, 0) == null) {
            android.widget.Toast.makeText(activity, R.string.lbl_headunit_missing, android.widget.Toast.LENGTH_LONG).show()
            return
        }
        activity.startActivity(Intent(activity, RequestActivity::class.java)
            .putExtra(RequestActivity.EXTRA_LAUNCH_HEADUNIT, true))
    }
    ...
    fun openSetup() {
        tryOpenActivity(Intent(Intent.ACTION_VIEW,
            Uri.parse("https://github.com/MaxGiuP/AAIdrive/blob/main/docs/android-auto-projection.md")))
    }
